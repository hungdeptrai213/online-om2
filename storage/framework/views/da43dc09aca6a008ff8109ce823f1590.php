...
<?php $__env->startSection('style'); ?>
    <style>
        ...
        .flipbook-stage {
            position: relative;
            background: linear-gradient(135deg, #faf6ef, #f0e8dc);
            border-radius: 18px;
    ...
    <div class="container-fluid px-2 px-md-3 px-lg-4">
        <div class="document-detail-modal">
            <div class="document-detail-header d-flex flex-wrap justify-content-between align-items-start gap-3 mb-4">
                ...
                <a href="<?php echo e(route('student.materials')); ?>" class="btn btn-outline-dark" style="border-radius:999px;">← Trở về kho tài liệu</a>
            </div>
                ...
                <div class="col-lg-9">
                    <div class="book-frame" id="bookFrame">
                        <div class="flipbook-toolbar">
                            <div class="nav-group">
                                <button type="button" class="btn btn-primary btn-prev">Previous page</button>
                                <span class="page-counter">[<span class="page-current">1</span> of <span class="page-total">-</span>]</span>
                                <button type="button" class="btn btn-primary btn-next">Next page</button>
                            </div>
                            <div class="flipbook-status">
                                State: <i class="page-state">—</i>, orientation: <i class="page-orientation">—</i>
                            </div>
                            <button id="fullscreenBtn" class="btn btn-outline-dark fullscreen-btn" aria-label="Toàn màn hình">
                        ...
                        <div class="flipbook-stage">
                            <div class="book-lock-overlay" id="lockOverlay">
                                ...
                                <a href="<?php echo e(route('student.documents.cart', ['document' => $document->id])); ?>" class="btn btn-success mt-2 px-4">Mua tài liệu</a>
                            </div>
                            <div class="flip-book" id="flipBook"></div>
                        </div>
                        <div id="pdfMessage" class="pdf-message">Đang tải tài liệu...</div>
                        <div class="text-center mt-3">
                            <a id="openOriginalLink" href="<?php echo e($originalLink); ?>" target="_blank" rel="noreferrer" class="text-decoration-underline small">Mở file gốc (trường hợp PDF không hiển thị)</a>
                        </div>
                    </div>
                </div>
                <div class="col-lg-3">
                    <div class="info-panel">
                        <img src="<?php echo e($document->thumbnail ?: asset('om-front/img/Open Document.png')); ?>"
                             alt="Ảnh đại diện <?php echo e($document->title); ?>">
                        <div>
                            <p class="fs-5 fw-semibold mb-1"><?php echo e($document->title); ?></p>
                            <p class="info-meta mb-1">Ngày phát hành: <?php echo e($document->published_at ? $document->published_at->format('d/m/Y') : 'Chưa rõ'); ?></p>
                            <p class="fs-5 mb-1">
                        ...
                        <div class="d-grid">
                            <?php if($document->price > 0): ?>
                                <?php if($isPurchased): ?>
                                    <a href="<?php echo e($document->link); ?>" target="_blank" rel="noreferrer" class="btn btn-success">Tải tài liệu bạn đã mua</a>
                                <?php else: ?>
                                    <a href="<?php echo e(route('student.documents.cart', ['document' => $document->id])); ?>" class="btn btn-primary">Mua tài liệu</a>
                                <?php endif; ?>
                            <?php else: ?>
                                <a href="<?php echo e($document->link); ?>" id="freeDownloadBtn" data-download-link="<?php echo e($originalLink); ?>" class="btn btn-success">Tải miễn phí</a>
                            <?php endif; ?>
                        </div>
                        <?php if($isPurchased && $document->price > 0): ?>
                            <p class="text-success small mb-0">Bạn đã thanh toán tài liệu này. Kiểm tra email để nhận file trực tiếp và hướng dẫn tải.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
...
<?php $__env->startPush('scripts'); ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const coverUrl = <?php echo json_encode($document->thumbnail ?: asset('om-front/img/Open Document.png'), 15, 512) ?>;
            ...
            if (!pdfUrl) {
                pdfMessageEl.textContent = 'Không tìm thấy đường dẫn PDF hợp lệ hoặc file chưa được chia sẻ công khai.';
                return;
            }
            ...
            const waitForPageFlipClass = (timeout = 12000) => new Promise((resolve, reject) => {
                ...
                const lookup = () => {
                    ...
                    if (Date.now() - start >= timeout) {
                        return reject(new Error('Không tải được PageFlip để hiển thị hiệu ứng lật trang.'));
                    }
            ...
            loadPdfJs()
                ...
                .then(async (doc) => {
                    ...
                    for (let i = 1; i <= previewPages; i++) {
                        ...
                        pageMeta.push({ type: 'page', src: rendered.dataUrl, alt: `Trang ${i}`, number: i });
                    }

                    if (isLocked && total > previewPages) {
                        lockPageIndex = pageMeta.length;
                        pageMeta.push({ type: 'lock' });
                    }
                ...
                .catch((err) => {
                    console.error(err);
                    pdfMessageEl.textContent = 'Không thể khởi tạo hoặc tải tài liệu PDF. Kiểm tra kết nối và đảm bảo file công khai.';
                    prevBtn.disabled = true;
                    nextBtn.disabled = true;
                    pageCurrentEl.textContent = '—';
                    pageTotalEl.textContent = '—';
...
<?php echo $__env->make('student.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\online-om\resources\views/student/documents/show.blade.php ENDPATH**/ ?>