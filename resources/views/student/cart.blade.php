                                ...
                                <tr id="cartRow">
                                    ...
                                    <td class="text-start">
                                        <div class="d-flex align-items-center">
                                            <img width="140" src="{{ $thumbUrl }}" alt="{{ $course->title }}" class="my-3 rounded-4 shadow-sm me-3">
                                            <span class="fs-3 course-title">{{ $course->title }}</span>
                                        </div>
                                    </td>
                                    <td class="text-nowrap price-cell" id="cartPrice">{{ $displayPrice }}</td>
                                    <td>
                                        <a class="btn btn-warning fs-4 rounded-4 fw-bold px-4 shadow-sm mt-3 mt-md-0" href="{{ route('student.cart') }}" role="button">Xóa</a>
                                    </td>
                                </tr>
                            @else
                                <tr id="emptyRow">
...
const courseDetailUrl = @json($course ? route('student.course-detail', ['course' => $course->id]) : route('student.home'));
const confirmUrl = @json($course ? route('student.cart.confirm') : null);
const csrfToken = '{{ csrf_token() }}';
const currentCourseId = {{ $course ? $course->id : 'null' }};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!autoCheckRan && {{ $course ? 'true' : 'false' }}) {
        autoCheckRan = true;
        setTimeout(() => {
            checkPaid({{ $rawPrice }}, '{{ $transferNoteSafe }}', null, true);
        }, 30000);
    }
});
</script>
