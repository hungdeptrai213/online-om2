...
@section('content')
    <div class="page-card">
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:14px;">
            ...
            <a href="{{ route('admin.users.create') }}" class="btn-dark">+ Thêm mới</a>
        </div>
        ...
        <form method="get" style="display:flex;justify-content:flex-end;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap;">
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Tìm theo tên hoặc email"
            ...
            <a href="{{ route('admin.users.index') }}" style="color:#1f2d3d;text-decoration:none;">Xóa lọc</a>
        </form>
        ...
        <div style="overflow:auto;">
            <table style="width:100%;border-collapse:collapse;">
                ...
                @forelse($users as $user)
                    <tr>
                        <td style="padding:12px;border-bottom:1px solid #f1f2f6;">{{ $user->id }}</td>
                        <td style="padding:12px;border-bottom:1px solid #f1f2f6;">{{ $user->name }}</td>
                        <td style="padding:12px;border-bottom:1px solid #f1f2f6;">{{ $user->email }}</td>
                        ...
                        <td style="padding:12px;border-bottom:1px solid #f1f2f6;">{{ $user->created_at?->format('d/m/Y') }}</td>
                        <td style="padding:12px;border-bottom:1px solid #f1f2f6;display:flex;gap:8px;flex-wrap:wrap;">
                            <a href="{{ route('admin.users.edit', $user) }}" style="padding:8px 10px;border-radius:8px;border:1px solid #e6ecf5;text-decoration:none;">Sửa</a>
