...
@section('content')
    <div class="page-card" style="max-width: 900px; margin: 0 auto;">
        ...
        <form method="post" action="{{ route('admin.courses.update', $course) }}" enctype="multipart/form-data" style="display:grid;gap:14px;">
            @csrf
            @method('PUT')
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="sale_price" value="{{ old('sale_price', $course->sale_price) }}"
                           style="width:100%;margin-top:6px;padding:10px 12px;border:1px solid #e3e8f1;border-radius:10px;">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="published_at" value="{{ old('published_at', optional($course->published_at)->format('Y-m-d\TH:i')) }}"
                           style="width:100%;margin-top:6px;padding:10px 12px;border:1px solid #e3e8f1;border-radius:10px;">
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;">
                ...
                <div>
                    ...
                    <label for="thumbInput" style="margin-top:6px;display:inline-flex;align-items:center;gap:8px;padding:10px 14px;border:1px solid #e3e8f1;border-radius:10px;background:#f8fafc;cursor:pointer;">
                        <i class="fa-solid fa-image"></i> <span id="thumbFileName">{{ $thumbUrl ? basename($thumbUrl) : 'Chọn ảnh' }}</span>
                    </label>
                    <div style="margin-top:8px;">
                        <img id="thumbPreview" src="{{ $thumbUrl }}" alt="Preview"
                             data-has-initial="{{ $thumbUrl ? '1' : '' }}"
                             style="max-width:200px;border-radius:10px;{{ $thumbUrl ? '' : 'display:none;' }}border:1px solid #e6ecf5;padding:4px;">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="short_description" rows="3" style="width:100%;margin-top:6px;padding:10px 12px;border:1px solid #e3e8f1;border-radius:10px;">{{ old('short_description', $course->short_description) }}</textarea>
            </div>

            <div>
                <label style="font-weight:700;">Nội dung chi tiết</label>
                <textarea name="description" rows="6" style="width:100%;margin-top:6px;padding:10px 12px;border:1px solid #e3e8f1;border-radius:10px;">{{ old('description', $course->description) }}</textarea>
            </div>
